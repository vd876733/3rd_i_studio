...
export default function Navigation() {
  ...
  return (
    <>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-slate-900 text-white dark:bg-primary dark:text-primary-foreground shadow-sm shadow-black/10"
                    : "text-slate-600 hover:text-slate-900 hover:bg-slate-200/50 dark:text-zinc-400 dark:hover:text-zinc-50 dark:hover:bg-zinc-800/50"
                )}
              >
                <Icon className={cn("w-5 h-5 transition-transform duration-200 group-hover:scale-105", isActive ? "text-white dark:text-primary-foreground" : "text-slate-500 dark:text-zinc-400")} />
        ...
        <div className="p-4 border-t border-[#e2e8f0] dark:border-zinc-800/80 bg-slate-50 dark:bg-zinc-950/40 space-y-3">
          ...
          <div className="flex items-center gap-3 p-2 rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/10">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-slate-800 dark:text-zinc-200 truncate">Administrator</p>
              <p className="text-[10px] text-slate-500 dark:text-zinc-400 truncate">admin@3rdistudio.com</p>
            </div>
            <button className="p-1.5 text-slate-500 hover:text-slate-800 dark:hover:text-zinc-200 rounded-lg hover:bg-zinc-200/50 dark:hover:bg-zinc-800/50 transition-colors">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 h-16 border-t border-[#e2e8f0] bg-white dark:border-zinc-800/80 dark:bg-zinc-955/70 z-20 flex items-center justify-around px-4 pb-safe">
        {NAV_ITEMS.map((item) => {
          const Icon = item.icon;
          const isActive = pathname === item.href;
          return (
