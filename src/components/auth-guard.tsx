...
export function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  if (status === "unauthenticated") {
    return (
      <div className="relative min-h-screen flex-1 flex flex-col overflow-hidden">
        {/* Blurred preview of the underlying dashboard */}
        <div className="absolute inset-0 select-none pointer-events-none filter blur-[3px]">
          {children}
        </div>

        {/* Backdrop overlay and modal card */}
        <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-md bg-slate-900/40 p-4">
          <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-md w-full text-center border border-slate-100 animate-in fade-in zoom-in duration-300">
            {/* Brand Header */}
            <div className="flex flex-col items-center mb-6">
              ...
              <p className="text-sm text-slate-500 mt-2 px-2">
                Please sign in with your studio Google account to access the dashboard and inventory.
              </p>
            </div>
