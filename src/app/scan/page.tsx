    ...
    <div className="flex-1 p-6 md:p-8 space-y-6 max-w-4xl mx-auto w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="relative aspect-video rounded-3xl bg-zinc-950 border border-zinc-900 overflow-hidden shadow-inner flex flex-col items-center justify-center">
            {/* Live Camera Stream */}
            <video 
              ref={videoRef} 
          ...
          <div className="flex flex-wrap items-center gap-3 p-4 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
            ...
            {devices.length > 1 && (
              <div className="flex-1 min-w-[180px] flex items-center gap-2">
                <span className="text-xs text-zinc-500 font-medium">Lens:</span>
                <select
        ...
        <div className="space-y-4">
          <div className="p-6 rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 h-full flex flex-col justify-between">
            <h3 className="font-semibold text-zinc-950 dark:text-zinc-50 text-sm">Decoded Item Specs</h3>
            
            <div className="flex-1 flex flex-col justify-center py-6">
              {isLoading ? (
                <div className="text-center space-y-3">
                  <RefreshCw className="w-8 h-8 text-cyan-500 animate-spin mx-auto" />
                  <p className="text-xs text-zinc-500">Querying database for item details...</p>
                </div>
              ) : itemData ? (
                <div className="space-y-4 text-left">
                  {/* Photo preview */}
                  <div className="relative aspect-video rounded-2xl overflow-hidden border border-zinc-150 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 shadow-sm">
                    <img
                  ...
                  <div className="space-y-1">
                    <code className="text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-wide block">{itemData.sku}</code>
                    <h4 className="text-md font-bold text-zinc-900 dark:text-zinc-50 leading-tight">{itemData.name}</h4>
                    <span className="inline-block text-[10px] font-semibold text-zinc-400 bg-zinc-100 dark:bg-zinc-950 px-2 py-0.5 rounded border border-zinc-150/50 dark:border-zinc-800/50 mt-1 font-mono">
                      {itemData.barcode}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-3 border-t border-zinc-100 dark:border-zinc-850">
                    <div className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800 flex items-start gap-2">
                      <MapPin className="w-4 h-4 text-cyan-500 shrink-0 mt-0.5" />
                      <div>
                        <span className="text-[9px] text-zinc-400 uppercase tracking-wider block">Rack & Shelf</span>
                        <span className="text-xs font-bold text-zinc-700 dark:text-zinc-300 block mt-0.5">
                    ...
                    <div className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800 flex items-start gap-2">
                      ...
                      <div>
                        <span className="text-[9px] text-zinc-400 uppercase tracking-wider block">Assigned Box</span>
                        <span className="text-xs font-bold text-zinc-700 dark:text-zinc-300 block mt-0.5 truncate">
                ...
                <div className="text-center space-y-3">
                  <div className="w-12 h-12 rounded-full bg-rose-50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-400 flex items-center justify-center mx-auto border border-rose-100 dark:border-rose-900/30">
                ...
                <div className="text-center space-y-2 p-6 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl bg-zinc-50/20 dark:bg-zinc-950/20">
                  <span className="text-xs text-zinc-400 italic block">No active barcode scan.</span>
                  <span className="text-[10px] text-zinc-500">Decoded barcode values will trigger instant DB query lookup.</span>
                </div>
              )}
            </div>

            {/* Quick manual scan input for testing if camera isn't accessible */}
            <div className="border-t border-zinc-100 dark:border-zinc-850 pt-4 mt-auto">
              <span className="text-[9px] text-zinc-400 uppercase tracking-wider block mb-1.5 font-bold">Manual Scan Simulation</span>
