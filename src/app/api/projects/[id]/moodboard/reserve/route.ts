...
export async function POST(
  ...
  try {
    ...
    // 3. Transaction to reserve item and log audit
    await prisma.$transaction([
      prisma.inventoryItem.update({
        where: { id: itemId },
        data: {
          currentStatus: InventoryStatus.RESERVED,
        },
      }),
      prisma.auditLog.create({
        data: {
          ...
          newValue: `Reserved item '${item.name}' (SKU: ${item.sku}) for Project '${project.name}' (${project.projectCode}) via Moodboard canvas placement`,
        },
      }),
    ]);

    return NextResponse.json({ success: true, itemId, status: InventoryStatus.RESERVED });
  } catch (error: any) {
    console.error("Error in Moodboard reserve API:", error);
