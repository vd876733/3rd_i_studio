  ...
  return (
    <div className="flex-1 p-6 md:p-8 space-y-8 max-w-7xl mx-auto w-full">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        {/* Total Inventory */}
        <div className="p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 shadow-sm space-y-3">
          <div className="p-2 w-fit rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400">
            <Package className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">Total Inventory</span>
            <span className="text-xl font-extrabold text-zinc-950 dark:text-zinc-50">{totalInventory}</span>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 shadow-sm space-y-3">
          ...
          <div>
            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">On Shoot</span>
            <span className="text-xl font-extrabold text-zinc-950 dark:text-zinc-50">{onShoot}</span>
          </div>
        </div>

        {/* Packed */}
        <div className="p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 shadow-sm space-y-3">
          <div className="p-2 w-fit rounded-lg bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400">
            <Layers className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">Packed Ready</span>
            <span className="text-xl font-extrabold text-zinc-950 dark:text-zinc-50">{packed}</span>
          </div>
        </div>

        {/* Missing/Damaged */}
        <div className="p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 shadow-sm space-y-3">
          <div className="p-2 w-fit rounded-lg bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400">
            <AlertTriangle className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">Lost / Damaged</span>
            <span className="text-xl font-extrabold text-zinc-950 dark:text-zinc-50">{missingDamaged}</span>
          </div>
        </div>

        {/* Today's Projects */}
        <div className="p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 shadow-sm space-y-3">
          <div className="p-2 w-fit rounded-lg bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400">
            <Calendar className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">Today's Shoots</span>
            <span className="text-xl font-extrabold text-zinc-950 dark:text-zinc-50">{todaysProjects}</span>
          </div>
        </div>

        {/* Overdue Returns */}
        <div className="p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 shadow-sm space-y-3">
          <div className="p-2 w-fit rounded-lg bg-purple-50 dark:bg-purple-950/30 text-purple-600 dark:text-purple-400">
            <Clock className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider block">Overdue Returns</span>
            <span className="text-xl font-extrabold text-zinc-950 dark:text-zinc-50">{overdueReturns}</span>
          </div>
        </div>
      </section>
          ...
          {PIPELINE_STAGES.map((stage) => {
            ...
            return (
              <div 
                key={stage.status}
                className={cn(
                  "p-4 rounded-2xl border bg-white/50 dark:bg-zinc-900/20 flex flex-col min-h-[300px] shadow-sm",
                  stage.borderClass
                )}
              >
                {/* Column Header */}
                  ...
                  {stageProjects.length > 0 ? (
                    stageProjects.map((project) => (
                      <Link
                        ...
                        className="p-3 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 hover:shadow-md transition-all duration-200 block text-left"
                      >
                        <span className="text-[9px] font-bold text-zinc-400 font-mono tracking-wide block mb-1">
                          {project.projectCode}
                        </span>
                        <h4 className="text-xs font-bold text-zinc-900 dark:text-zinc-100 leading-snug line-clamp-2">
                          {project.name}
                        </h4>
                        <div className="flex flex-col gap-1 mt-2.5 pt-2.5 border-t border-zinc-100 dark:border-zinc-800">
                    ...
                    <div className="flex-1 flex items-center justify-center p-4 text-center border border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl">
                      <span className="text-[10px] text-zinc-400 italic">No Projects</span>
                    </div>
                  )}
                </div>
              </div>
